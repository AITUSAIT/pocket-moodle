...
async def courses_contents_course_content_module(query: types.CallbackQuery):
    ...
    await query.message.edit_reply_markup(
        reply_markup=back_btn(
            ...
            kb=url_btns(urls=urls, kb=files_btns(files=files, course_id=course_id, module_id=module_id)),
        ).as_markup()
    )
...
async def courses_send_file(query: types.CallbackQuery):
    ...
    file_bytes = await PocketMoodleAPI().get_course_content_module_file_bytes(course_id, module_id, file_id)

    await query.message.answer_document(
        document=types.BufferedInputFile(file=file_bytes.getvalue(), filename=file.filename),
    )
...
def register_handlers_courses_contents(router: Router):
    ...
    router.callback_query.register(
        courses_send_file,
        F.func(lambda c: c.data.split()[0] == "courses_contents"),
        F.func(lambda c: len(c.data.split()) == 5),
        F.func(lambda c: c.data.split()[1] == "file"),
    )
