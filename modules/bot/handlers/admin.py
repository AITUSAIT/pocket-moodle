...
def register_handlers_admin(router: Router):
    if TEST:
        router.message.register(ignore, IsNotStuff(), F.text)

    personalChatTypeFilter = ChatTypeFilter(chat_type=["sender", "private"])

    router.message.register(send_msg, IsAdmin(), personalChatTypeFilter, Command("send_msg"))
    router.message.register(get, IsManager(), personalChatTypeFilter, Command("get"))
    router.message.register(deanon, IsManager(), F.func(lambda msg: msg.reply_to_message), Command("deanon"))
