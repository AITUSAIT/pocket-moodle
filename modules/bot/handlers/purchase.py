...
from config import RATE, prices
...
from modules.bot.keyboards.purchase import payment_btn, periods_btns, purchase_btns
from modules.database import PaymentDB, PromocodeDB, UserDB
from modules.database.models import User
from modules.logger import Logger
from modules.oxapay import OxaPay, Transaction
...
@Logger.log_msg
async def purchase(query: types.CallbackQuery):
...
@dp.throttled(rate=RATE)
async def purchase_sub(query: types.CallbackQuery):
    await query.answer()
...
@dp.throttled(rate=RATE)
async def create_payment(query: types.CallbackQuery):
    await query.answer()
...
async def enter_promocode(message: types.Message, state: FSMContext):
    ...
    if usage_settings == "newbie" and not user.is_newbie():
...
def register_handlers_purchase(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(purchase, lambda c: c.data == "purchase", state="*")

    dp.register_callback_query_handler(purchase_sub, lambda c: c.data in ("purchase_sub", "purchase_promo"), state="*")
    dp.register_callback_query_handler(
        create_payment,
        lambda c: c.data.split("|")[0] == "purchase_sub" or c.data.split("|")[0] == "purchase_promo",
        state="*",
    )
