...
class PaymentDB(DB):
    ...
    async def create_payment(cls, transaction: Transaction) -> None:
        async with cls.pool.acquire() as connection:
            await connection.execute(
                ...
                transaction["is_for_promocode"],
                False,
            )

    @classmethod
    async def delete_payment(cls, track_id: int) -> None:
        async with cls.pool.acquire() as connection:
            await connection.execute("DELETE FROM user_payment WHERE trackId = $1", track_id)
    ...
    async def update_payment(cls, transaction: Transaction) -> None:
        async with cls.pool.acquire() as connection:
            await connection.execute(
                "UPDATE user_payment SET verified = $1 WHERE trackId = $2;", True, transaction["trackId"]
            )
